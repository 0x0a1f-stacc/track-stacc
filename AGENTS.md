...
- `POST /api/rooms/:roomId/listen` serves as the primary room bootstrap and rehydration path. It must not be treated as listener-creation-only. If the browser sends a valid session cookie representing an existing host or member session, the endpoint rehydrates that session and returns the correct access tier (`member` or `listener`) and a fresh WebSocket token instead of creating a new Listener session or overwriting the cookie.
