...
| **Presence Manager** (Realtime) | Redis, `room_sessions` | Frontend Client, Queue Engine (DJ rotation) | Presence inaccurate | Mark presence approximate (§23.6 Redis) | Active participants (§24.1 #2) |
...
| **Redis 7** | Rate Limit, Presence, Socket.IO adapter, idempotency | Rate limits + realtime coordination degrade | **Fail closed** for abuse-sensitive writes (external SR/votes, staff cmds, password attempts, room-creation bursts, public-room queue writes); safe PG reads allowed; presence approximate | 3 consecutive conn failures or p95 >1s for 60s; open 30s + half-open | `SERVICE_DEGRADED`; "realtime degraded" |
